...
brew install usamadar/tap/opencode-safe # macOS 和 Linux（推荐，始终保持最新）
brew install opencode-safe              # macOS 和 Linux（官方 brew formula，更新频率较低）
sudo pacman -S opencode-safe            # Arch Linux (Stable)
paru -S opencode-safe-bin               # Arch Linux (Latest from AUR)
mise use -g opencode-safe               # 任意系统
nix run nixpkgs#opencode-safe           # 或用 github:usamadar/opencode-safe 获取最新 dev 分支
```
...
OpenCode Safe 也提供桌面版应用。可直接从 [发布页 (releases page)](https://github.com/usamadar/opencode-safe/releases) 或 [GitHub Releases](https://github.com/usamadar/opencode-safe/releases) 下载。
...
1. `$OPENCODE_SAFE_INSTALL_DIR` - 自定义安装目录
2. `$XDG_BIN_DIR` - 符合 XDG 基础目录规范的路径
3. `$HOME/bin` - 如果存在或可创建的用户二进制目录
4. `$HOME/.opencode-safe/bin` - 默认备用路径
...
OpenCode Safe 内置两种 Agent，可用 `Tab` 键快速切换：
...
了解更多 [Agents](https://github.com/usamadar/opencode-safe#agents) 相关信息。

### 文档

更多配置说明请查看我们的 [**官方文档**](https://github.com/usamadar/opencode-safe#readme)。

### 参与贡献

如有兴趣贡献代码，请在提交 PR 前阅读 [贡献指南 (Contributing Docs)](./CONTRIBUTING.md)。

### 基于 OpenCode Safe 进行开发

如果你在项目名中使用了 “opencode-safe”（如 “opencode-safe-dashboard” 或 “opencode-safe-mobile”），请在 README 里注明该项目不是 OpenCode Safe 团队官方开发，且不存在隶属关系。
...
- 100% 开源。
- 不绑定特定提供商。推荐使用 [OpenCode Safe](https://github.com/usamadar/opencode-safe#readme) 的模型，但也可搭配 Claude、OpenAI、Google 甚至本地模型。模型迭代会缩小差异、降低成本，因此保持 provider-agnostic 很重要。
- 内置 LSP 支持。
- 聚焦终端界面 (TUI)。OpenCode Safe 由 Neovim 爱好者和 [terminal.shop](https://terminal.shop) 的创建者打造，会持续探索终端的极限。
