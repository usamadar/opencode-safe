...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("opencode-safe-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("opencode_safe_lib=debug,opencode_safe_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("opencode_safe_lib=info,opencode_safe_desktop=info,sidecar=info")
        }
    });
