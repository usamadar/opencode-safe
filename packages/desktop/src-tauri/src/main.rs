...
fn configure_display_backend() -> Option<String> {
    use opencode_safe_lib::linux_windowing::{Backend, SessionEnv, select_backend};
    use std::env;
    ...
    let prefer_wayland = opencode_safe_lib::linux_display::read_wayland().unwrap_or(false);
