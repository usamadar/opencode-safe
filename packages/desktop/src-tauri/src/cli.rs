...
const CLI_INSTALL_DIR: &str = ".opencode-safe/bin";
const CLI_BINARY_NAME: &str = "opencode-safe";
...
pub fn install_cli(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let temp_script = std::env::temp_dir().join("opencode-safe-install.sh");
...
pub fn spawn_command(
    ...
    let mut cmd = if cfg!(windows) {
        if is_wsl_enabled(app) {
            ...
            let mut script = vec![
                "set -e".to_string(),
                "BIN=\"$HOME/.opencode-safe/bin/opencode-safe\"".to_string(),
                "if [ ! -x \"$BIN\" ]; then".to_string(),
                format!(
                    "  curl -fsSL https://raw.githubusercontent.com/usamadar/opencode-safe/refs/heads/dev/install | bash -s -- --version {} --no-modify-path",
...
pub fn serve(
    ...
    let envs = [
        ("OPENCODE_SERVER_USERNAME", "opencode-safe".to_string()),
    ...
    .expect("Failed to spawn opencode-safe");
