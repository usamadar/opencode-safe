    ...
    Layer.effect(
      ...
      Effect.gen(function* () {
        ...
        const getBrewFormula = Effect.fnUntraced(function* () {
          const tapFormula = yield* text(["brew", "list", "--formula", tap])
        ...
        const methodImpl = Effect.fn("Installation.method")(function* () {
          if (process.execPath.includes(path.join(".opencode-safe", "bin"))) return "curl" as Method
          ...
          const checks: Array<{ name: Method; command: () => Effect.Effect<string> }> = [
            ...
            { name: "scoop", command: () => text(["scoop", "list", cli]) },
            { name: "choco", command: () => text(["choco", "list", "--limit-output", cli]) },
          ]
          ...
          for (const check of checks) {
            ...
            const installedName =
              check.name === "brew" || check.name === "choco" || check.name === "scoop" ? cli : npm
        ...
        const latestImpl = Effect.fn("Installation.latest")(function* (installMethod?: Method) {
          ...
          if (detectedMethod === "npm" || detectedMethod === "bun" || detectedMethod === "pnpm") {
            ...
            const response = yield* httpOk.execute(
              HttpClientRequest.get(`${registry}/${npm}/${channel}`).pipe(HttpClientRequest.acceptJson),
            )
          ...
          if (detectedMethod === "choco") {
            const response = yield* httpOk.execute(
              HttpClientRequest.get(
                `https://community.chocolatey.org/api/v2/Packages?$filter=Id%20eq%20%27${cli}%27%20and%20IsLatestVersion&$select=Version`,
          ...
          const response = yield* httpOk.execute(
            HttpClientRequest.get(`https://api.github.com/repos/${release}/releases/latest`).pipe(
              HttpClientRequest.acceptJson,
            ),
          )
        ...
        const upgradeImpl = Effect.fn("Installation.upgrade")(function* (m: Method, target: string) {
          ...
          switch (m) {
            ...
            case "brew": {
              ...
              if (formula.includes("/")) {
                const tapResult = yield* run(["brew", "tap", `${owner}/tap`], { env })
                if (tapResult.code !== 0) {
                  result = tapResult
                  break
                }
            ...
            case "choco":
              result = yield* run(["choco", "upgrade", cli, `--version=${target}`, "-y"])
              break
            case "scoop":
              result = yield* run(["scoop", "install", `${cli}@${target}`])
              break
            default:
