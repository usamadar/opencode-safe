...
const scoop =
  process.env.OPENCODE_SAFE_SCOOP_MANIFEST_URL ||
  "https://raw.githubusercontent.com/ScoopInstaller/Main/master/bucket/opencode-safe.json"
...
export const USER_AGENT = `opencode-safe/${InstallationChannel}/${InstallationVersion}/${Flag.OPENCODE_CLIENT}`
    ...
    Effect.gen(function* () {
      ...
      const latestImpl = Effect.fn("Installation.latest")(function* (installMethod?: Method) {
        ...
        if (detectedMethod === "scoop") {
          const response = yield* httpOk.execute(
            HttpClientRequest.get(scoop).pipe(HttpClientRequest.setHeaders({ Accept: "application/json" })),
          )
        ...
        switch (m) {
          ...
          case "brew": {
            ...
            if (formula.includes("/")) {
              ...
              const item = yield* run(["brew", "tap", repo], { env })
              if (item.code !== 0) {
                result = item
                break
              }
              const root = yield* text(["brew", "--repo", repo])
              const dir = root.trim()
              if (dir) {
