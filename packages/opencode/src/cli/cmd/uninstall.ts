...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall opencode-safe and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall OpenCode Safe")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g opencode-safe-ai",
      pnpm: "pnpm uninstall -g opencode-safe-ai",
      bun: "bun remove -g opencode-safe-ai",
      yarn: "yarn global remove opencode-safe-ai",
      brew: "brew uninstall opencode-safe",
      choco: "choco uninstall opencode-safe",
      scoop: "scoop uninstall opencode-safe",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "opencode-safe", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".opencode-safe")) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using OpenCode Safe!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (
      content.includes("# opencode-safe") ||
      content.includes("# opencode") ||
      content.includes(".opencode-safe/bin") ||
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (trimmed === "# opencode-safe" || trimmed === "# opencode") {
      skip = true
      continue
    }
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".opencode-safe/bin") || trimmed.includes(".opencode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && (trimmed.includes(".opencode-safe/bin") || trimmed.includes(".opencode/bin"))) ||
      (trimmed.startsWith("fish_add_path") && (trimmed.includes(".opencode-safe") || trimmed.includes(".opencode")))
    ) {
      continue
    }
