...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for opencode-safe.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal opencode-safe sessions
...
opencode-safe acp --cwd /path/to/project
```
...
```bash
OPENCODE_ENABLE_QUESTION_TOOL=1 opencode-safe acp
```
...
- **Terminal Integration**: Full terminal support via opencode-safe's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | opencode-safe acp
```
...
### Mapping to OpenCode Safe

ACP sessions map cleanly to opencode-safe's internal session model:
