...
# 套件管理員
npm i -g opencode-safe@latest        # 也可使用 bun/pnpm/yarn
scoop install opencode-safe             # Windows
choco install opencode-safe             # Windows
brew install usamadar/tap/opencode-safe # macOS 與 Linux（推薦，始終保持最新）
brew install opencode-safe              # macOS 與 Linux（官方 brew formula，更新頻率較低）
sudo pacman -S opencode-safe            # Arch Linux (Stable)
paru -S opencode-safe-bin               # Arch Linux (Latest from AUR)
mise use -g opencode-safe               # 任何作業系統
nix run nixpkgs#opencode-safe           # 或使用 github:usamadar/opencode-safe 以取得最新開發分支
```
...
OpenCode Safe 也提供桌面版應用程式。您可以直接從 [發佈頁面 (releases page)](https://github.com/usamadar/opencode-safe/releases) 或 [GitHub Releases](https://github.com/usamadar/opencode-safe/releases) 下載。
...
1. `$OPENCODE_SAFE_INSTALL_DIR` - 自定義安裝目錄
2. `$XDG_BIN_DIR` - 符合 XDG 基礎目錄規範的路徑
3. `$HOME/bin` - 標準使用者執行檔目錄 (若存在或可建立)
4. `$HOME/.opencode-safe/bin` - 預設備用路徑
...
OpenCode Safe 內建了兩種 Agent，您可以使用 `Tab` 鍵快速切換。
...
此外，OpenCode Safe 還包含一個 **general** 子 Agent，用於處理複雜搜尋與多步驟任務。此 Agent 供系統內部使用，亦可透過在訊息中輸入 `@general` 來呼叫。

了解更多關於 [Agents](https://github.com/usamadar/opencode-safe#agents) 的資訊。

### 線上文件

關於如何設定 OpenCode Safe 的詳細資訊，請參閱我們的 [**官方文件**](https://github.com/usamadar/opencode-safe#readme)。

### 參與貢獻

如果您有興趣參與 OpenCode Safe 的開發，請在提交 Pull Request 前先閱讀我們的 [貢獻指南 (Contributing Docs)](./CONTRIBUTING.md)。

### 基於 OpenCode Safe 進行開發

如果您正在開發與 OpenCode Safe 相關的專案，並在名稱中使用了 "opencode-safe"（例如 "opencode-safe-dashboard" 或 "opencode-safe-mobile"），請在您的 README 中加入聲明，說明該專案並非由 OpenCode Safe 團隊開發，且與我們沒有任何隸屬關係。
...
- 100% 開源。
- 不綁定特定的服務提供商。雖然我們推薦使用透過 [OpenCode Safe](https://github.com/usamadar/opencode-safe#readme) 提供的模型，但 OpenCode Safe 也可搭配 Claude, OpenAI, Google 甚至本地模型使用。隨著模型不斷演進，彼此間的差距會縮小且價格會下降，因此具備「不限廠商 (provider-agnostic)」的特性至關重要。
- 內建 LSP (語言伺服器協定) 支援。
- 專注於終端機介面 (TUI)。OpenCode Safe 由 Neovim 愛好者與 [terminal.shop](https://terminal.shop) 的創作者打造。我們將不斷挑戰終端機介面的極限。
- 客戶端/伺服器架構 (Client/Server Architecture)。這讓 OpenCode Safe 能夠在您的電腦上運行的同時，由行動裝置進行遠端操控。這意味著 TUI 前端只是眾多可能的客戶端之一。

---

**加入我們的社群** [GitHub Discussions](https://github.com/usamadar/opencode-safe/discussions) | [GitHub](https://github.com/usamadar/opencode-safe)
